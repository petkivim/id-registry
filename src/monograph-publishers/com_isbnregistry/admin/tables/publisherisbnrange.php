 ...
 * @since  1.0.0
 */
class IsbnRegistryTablePublisherisbnrange extends JTable {
    ...
    function __construct(&$db) {
        parent::__construct('#__isbn_registry_publisher_isbn_range', 'id', $db);
    }
    ...
    public function saveToDb($isbnrange, $publisherId, $publisherIdentifier) {
        ...
        $this->isbn_range_id = $isbnrange->id;
     ...
     * @return boolean true on success; false on failure
     */
    public function updateToDb($publisherIsbnrange) {
        // Load object
        if (!$this->load($publisherIsbnrange->id)) {
            return false;
        }

        // Update fields
        $this->free = $publisherIsbnrange->free;
        $this->taken = $publisherIsbnrange->taken;
        $this->next = $publisherIsbnrange->next;
        $this->is_active = $publisherIsbnrange->is_active;
        $this->is_closed = $publisherIsbnrange->is_closed;
